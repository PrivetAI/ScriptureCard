...
//      `SCPanelSession` remembers where they actually were.
...
//      persistent store does not help. `SCPanelCookies` mirrors the jar out and
...
enum SCPanelSession {
    private static let addressKey = "sc.panel.resume.address"
    private static let stampKey   = "sc.panel.resume.stamp"
...
enum SCPanelCookies {
    private static let key = "sc.panel.cookies"
...
struct SCWebPanel: UIViewRepresentable {
    ...
    final class Coordinator: NSObject, WKNavigationDelegate {
        ...
        func watchAddress(of webView: WKWebView) {
            ...
            urlObservation = webView.observe(\.url, options: [.new]) { [weak self] webView, _ in
                ...
                SCPanelSession.remember(webView.url, trackerHost: self.trackerHost)
            }
        }

        // didCommit, not didFinish — didFinish lands seconds after the page is usable.
        func webView(_ webView: WKWebView, didCommit navigation: WKNavigation!) {
            SCPanelSession.remember(webView.url, trackerHost: trackerHost)
            fire()
        }
    ...
    func makeUIView(context: Context) -> WKWebView {
        ...
        SCPanelCookies.restore { [weak webView] in
